...
class Sentiment():
	...
	def get_feature(self, method="tfidf"):
		if method == "tfidf":
			...
			vectorizer = TfidfVectorizer(ngram_range=(1,1), max_df=0.7, min_df=10, vocabulary=selected_names)
			...
			save_vocab = True
			if save_vocab:
				with open("en_sentiment_vocab.txt", "w", encoding="utf8") as f:
					for w in selected_names:
						f.write(w + "\n")

			save_transformer = True
			if save_transformer:
				with open("en_sentiment_transformer.pkl", "wb") as f:
	...
	def train(self, dataPath, modelName='LR'):
		...
		if modelName == "LR":
			...
			save_model = True
			if save_model:
				with open("en_sentiment_model_svm.pkl", "wb") as f:
					pickle.dump(model, f)
	...
	def infer(self):
		...
		print("start to get the feature ...")
		#self.get_feature()
		vectorizer = pickle.load(open("en_sentiment_transformer.pkl", "rb"))
		model = pickle.load(open("en_sentiment_model_svm.pkl", "rb"))
		results = model.predict(self.test_tfidf)
		print(classification_report(self.Y_test, results, digits=4))
